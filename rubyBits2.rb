...
#Рассмотри прменение динамических инструментов работы с классами, на примере  создания метода-логгера. Данный метод будет принимать в себя аргумент имя класса и имя метода, который будет исполнятся. Далее метод будет создавать оригинальную копию исполняемого метода и фиксировать время последнего исполнения. 
...
# в примере выше, мы видим, что в аргемунте передается объект user, и мы вызываем его методы username и avatar.
...
#----------------- DYNAMIC METHODS -----------------------------
# динамические методы можно использовать, когда нам известно часть имени методов, некая маска у определенной группы методов, и мы можем вызывать такие методы, с помощью указания параметров этой маски
...
class Tweet
  ...
  def method_missing(method_name, *args)
    ...
    # метод match - возвращает значение, которое соответствует регулярному выражению, заданному в арнументе. Если же заданное значение не найдено, то метод возвращает nil (https://goo.gl/xBuud4)
...
# решение этой ситуации доступно с версии Ruby 1.9.3, с помощью переопределения метода respond_to_missing ?
...
class Tweet
  ...
  # метод формирует строку из наполненного массива, сам маасив наполняется ниже...
...
#----------------- NAMESPACE POLLUTION -----------------------------
# но вышеуказанный способ не сильно подходит, потому что, синтаксис не соответствует стандартам DSL, вызов к полям передаваемого блока проводится через заданный объект tweet.
